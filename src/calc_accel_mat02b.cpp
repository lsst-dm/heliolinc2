// July 21, 2023: calc_accel_mat02b:
// Like calc_accel_mat02, but uses a more efficient sampling scheme.
...
int main(int argc, char *argv[])
{
  ...
  while(i<argc) {
    ...
    }  else if(string(argv[i]) == "-accelover" || string(argv[i]) == "-ao" || string(argv[i]) == "-accover" || string(argv[i]) == "-acco" || string(argv[i]) == "--accelover" ) {
      if(i+1 < argc) {
	//There is still something to read;
	accel_oversamp = stod(argv[++i]);
	i++;
      }
      else {
	cerr << "Acceleration oversampling keyword supplied with no corresponding argument\n";
	show_usage();
	return(1);
      }
  ...
  while(dist<=maxdist && stepct<=STEPMAX) {
    ...
    velstep = diststep*AU_KM/2.0/timescale/SOLARDAY; // km/sec
    g0 = GMSUN_KM3_SEC2/distkm/distkm; // Solar gravity in km/sec^2
    accelstep = velstep/timescale/SOLARDAY/accel_oversamp; // Acceleration required to change velocity
                                             // by one velocity step over the characteristic timescale
    if(verbose>=1) cout << "Distance is " << dist << " AU = " << distkm << " km, vesc = " << vesc << " km/sec, velstep = " << velstep << "km/sec, gt = " << g0*timescale*SOLARDAY << "\n";
