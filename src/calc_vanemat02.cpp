...
{
  cerr << "Usage: calc_vanemat02 -minheliodist minimum distance from sun (AU) -mingeodist min distance from Earth (AU) -distfracstep distances sampling interval as a fraction of geodist -clustrad clustering radius (km) -velscale undersampling factor for velocity -totaltimespan total time spanned by the data (days) -minsunelong minimum solar elongation (deg) -outfile output file\n";
...
int main(int argc, char *argv[])
{
  ...
  while(lambda<=coarselimit) {
    ...
    dot_step = 2.0l*dist*vel_undersamp*(clustrad/AU_KM)*DEGPRAD/totaltime;
	...
	ddot_step = 4.0l*dot_step/totaltime;
