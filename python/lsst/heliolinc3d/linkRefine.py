...
#LinkRefineMerge
class LinkRefineConnections(lsst.pipe.base.PipelineTaskConnections,
                           dimensions=("instrument", "visitWindow", "hypothesis", "orbitType"),
    ...
    trackletSources = connectionTypes.Input(
     ...
     summaryRefined = connectionTypes.Output(
        doc="one line summary of each linkage",
        dimensions=("instrument", "visitWindow"),
        storageClass="SourceCatalog",
        name = "{hypothesis}_hlclustRefined_{orbitType}",
     )
     clust2detRefined = connectionTypes.Output(
        doc="indices connecting linkages (clusters) to trackletSources",
        dimensions=("instrument", "visitWindow"),
        storageClass="SourceCatalog",
        name = "{hypothesis}_clust2detRefined_{orbitType}",
     )
...
class LinkRefineTask(lsst.pipe.base.PipelineTask):
    ...
    def run(self, visitTable, trackletSources, summary, clust2det):
        ...
        return lsst.pipe.base.Struct(summaryRefined=linkout[0],
                                     clust2detRefined=linkout[1]
                                     )
