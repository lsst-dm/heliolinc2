...
int main(int argc, char *argv[])
{
  ...
  long double barydist,obsdot,opdistcos,opelong,sunelong;
  ...
  while(reachedeof==0) {
	...
	planetpos01LD(MJD, 5, EarthMJD, Earthpos, outpos);
	oldMJD=MJD;
      }
      celestial_to_stateunitLD(RA,Dec,unitbary);      
      barydist = sqrt(dotprod3LD(outpos,outpos));
      obsdot = dotprod3LD(unitbary,outpos);
