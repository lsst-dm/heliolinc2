...
def test_transforms():
    ...
    # 367943 Duende
    duende_ecliptic_state=np.array([6.812494160509899E-01, -7.600337887823437E-01,  4.884690866200464E-02,
                                     1.115613719147700E-02,  1.227412234901252E-02, -2.983658410793795E-03])
    
    duende_icrf_state=np.array([6.812494160509899E-01, -7.167475521779160E-01, -2.575079164290766E-01,
                                1.115613719147700E-02,  1.244811823977932E-02,  2.144912408305390E-03])
    # q,e,i,node,w,tp
    duende_cometary_orbital_elements=np.array([8.903562405687978E-01,1.107475730313936E-01,1.033215876778472E+01,
                                                1.470901133231629E+02, 2.714274888596638E+02, 2456261.459543305449]) 
    # a,e,i,w,node,M
    duende_keplerian_orbital_elements=np.array([1.001241282640019E+00, 1.107475730313936E-01,  1.033215876778472E+01,
                                                2.714274888596638E+02,1.470901133231629E+02, 2.655340479552559E+02])                                                             
    # Right Ascension and Declination in UTC
    duende_radec_ut=np.array([313.540503119, -14.596773076])
    
    # WARNING: JPL dRA/dt and dDEC/dt are not in ICRF! They can only serve as order of magnitude checks.
    duende_dradec_dt_ut=np.array([140.1392,  14.62131])/3600*24
    
    duende_r_rdot=np.array([1.02184018134897, -3.1759900/au2km*86400])
    ...
    check2(tst,correct_solution,rtol=1e-14, atol=1e-15)
...
#     correct_solution=np.hstack([ceres_radec_ut,ceres_dradec_dt_ut,ceres_r_rdot])
    ...
    print('367943 Duende')
    tst=ephem.icrf2ephemeris(epoch,duende_icrf_state,timescale_epoch='utc',timescale_state='tdb',deg=True,lttc=True)
    correct_solution=np.hstack([duende_radec_ut,duende_dradec_dt_ut,duende_r_rdot])
