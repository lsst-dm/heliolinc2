...
int main(int argc, char *argv[])
{
  ...
  vector <point6LDx2> allstatevecs;
  long double cluster_radius = 1e5L;
  KD_point6LDx2 kdpoint = KD_point6LDx2(statevec1,-1,-1,1,0);
  vector <KD_point6LDx2> kdvec;
  long kdroot=0;
  long splitpoint=0;
  long double chartimescale = 1e5L;
  long double minMJD = 1e30L;
  long double maxMJD = -1e30L;
  vector <KD6_clust> outclusters;
  ...
  for(accelct=0;accelct<accelnum;accelct++) {
    ...
    kdvec={};
    kdroot = splitpoint = 0;
    splitpoint=medind_6LDx2(allstatevecs,1);
    kdpoint = KD_point6LDx2(allstatevecs[splitpoint],-1,-1,1,0);
    kdvec.push_back(kdpoint);
    kdtree_6D01(allstatevecs,1,splitpoint,kdroot,kdvec);
    ...
    outclusters={};
    int npt=3;
    int clusternum = DBSCAN_6D02(kdvec, cluster_radius, npt, outclusters);
    cout << "DBSCAN_6D02 finished, with " << clusternum << " = " << outclusters.size() << " clusters found\n";
    realclusternum=0;
