...
int main()
{
  ...
  point3d startvel2 = point3d(0.0L,0.0L,0.0L);
  ...
  for(testct=0; testct<testnum; testct++) {
    ...
    startvel = Twopoint_Kepler_v1(MGsun, startpos, endpos, timediff, 1.0, &a, 150, 0);
    int status = Twopoint_Kepler_vstar(MGsun, startpos, endpos, timediff, startvel2, KEPTRANSITMAX);
    if(status!=0) {
      cerr << "Warning: Twopoint_Kepler_vstar() returned error status " << status << "\n";
    }
    cout << "Twopoint_Kepler_v1() finds initial velocity " << startvel.x << " " << startvel.y << " " << startvel.z << ", a = " << a/AU_KM << "\n";
    if(startvel.x==0.0l) return(3);
    cout << "Twopoint_Kepler_vstar() finds initial velocity " << startvel2.x << " " << startvel2.y << " " << startvel2.z << "\n";
    cout << "diffs: " << startvel.x-startvel2.x << " " << startvel.y-startvel2.y << " " << startvel.z-startvel2.z << "\n";
    outstream1 << startvel.x << " " << startvel.y << " " << startvel.z << " " << a/AU_KM << " " << startvel2.x << " " << startvel2.y << " " << startvel2.z << " " << startvel.x-startvel2.x << " " << startvel.y-startvel2.y << " " << startvel.z-startvel2.z << "\n";
  }
  outstream1.close();
  return(0);
}
