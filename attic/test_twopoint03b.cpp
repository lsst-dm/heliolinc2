...
int main()
{
  ...
  for(testct=0; testct<testnum; testct++) {
    ...
    Kepler_fg_func_int(MGsun, 0.0l, startpos, startvel, timediff, outpos, endvel);
    ...
    outstream1 << endpos.x << " " << endpos.y << " " << endpos.z << " " << outpos.x << " " << outpos.y << " " << outpos.z << " " << endpos.x-outpos.x << " " << endpos.y-outpos.y << " " << endpos.z-outpos.z << "\n";
  }
  outstream1.close();
  return(0);
}
