...
int main()
{
  ...
  for(testct=0; testct<testnum; testct++) {
    ...
    if(status!=0) {
      cerr << "Warning: Twopoint_Kepler_vel() returned error status " << status << "\n";
    }
    if(a>0.0) {
      cout << "Twopoint_Kepler_vel() finds an elliptical orbit with a = " << a/AU_KM << "\n";
      cout << "and initial velocity " << startvel.x << " " << startvel.y << " " << startvel.z << "\n";
    }
    else if(a<0.0) {
      cout << "Twopoint_Kepler_vel() finds a hyperbolic orbit with a = " << a/AU_KM << "\n";
      cout << "and initial velocity " << startvel.x << " " << startvel.y << " " << startvel.z << "\n";
    }
