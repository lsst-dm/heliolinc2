// January 17, 2022: calc_accel_mat01a.cpp
...
{
  cerr << "Usage: calc_accel_mat01a -orbfile orbit_file -outfile output_file -diststep dist0 diststep distnum -velstep vel0 velstep velnum -accsplit nsplit -timestep timestep timenum\n";
}
...
int main(int argc, char *argv[])
{
  ...
  while(i<argc) {
    ...
    }  else if(string(argv[i]) == "-accsplit" || string(argv[i]) == "-acc"  || string(argv[i]) == "-accelsplit" || string(argv[i]) == "-as" || string(argv[i]) == "--acceleration" || string(argv[i]) == "--accelsplit" || string(argv[i]) == "--accelerationsplit") {
      if(i+1 < argc) {
	//There is still something to read;
	accelsplit=stoi(argv[++i]);
	i++;
      }
      else {
  ...
  cout << "acceleration split " << accelsplit << "\n";
  ...
  for(distct=0; distct<distnum; distct++) {
	...
	for(i=1;i<=accelsplit;i++) {
	  j = i*accelvec.size()/(i+1);
	  outstream1 << fixed << setprecision(6) << gridvec[distct*velnum+velct].dist << " " << gridvec[distct*velnum+velct].vel << " " << accelvec.size() <<  " " << accelvec[j] << " -99.9 -99.9 -99.9\n";
	}
      }
    }
  }
