// January 13, 2022: count_findable01a.cpp
...
int main(int argc, char *argv[])
{
  ...
  while(detct<detvec.size()) {
    ...
    if(detct<detvec.size()) {
      //cout << "Working on " << idstring << ", " << trackvec.size() << " points.\n";
	...
	for(i=1; i<trackvec.size(); i++) {
	  ...
	  angdist = distradec01(trackvec[i-1].RA, trackvec[i-1].Dec, trackvec[i].RA, trackvec[i].Dec); // Distance in degrees
	  angspeed = angdist/currentstep; // Angular velocity in deg/day
	  //cout << "Det " << i << " step = " << currentstep << " angdist, vel = " << angdist << " " << angspeed;
	  if(currentstep>MINOBSINTERVAL/SOLARDAY && currentstep<=maxtime && angdist<=maxdist && angspeed<=maxvel) {
	    // Detections i-1 and i constitiute a valid tracklet,
	    // safely contained within a single night, and meeting
	    // requirements on maximum separation and angular velocity.
	    trackletcount+=1;
	    trackletmjd.push_back(trackvec[i].MJD*0.5L + trackvec[i-1].MJD*0.5L);
	    //cout << " qualifies, tracklet " << trackletcount << "\n";
	  } else {
	...
	if(tracknightcount >= MINDAYSTEPS) {
	  ...
	  outstream1  << fixed << setprecision(3) << r0 << " " << heliovel << " ";
	  outstream1  << fixed << setprecision(6) << a << " " << a << " " << a << "\n";
	}
	...
	outstream1  << fixed << setprecision(6) << a << " " << a << " " << a << "\n";
      ...
      idstring = detvec[detct].idstring;
      trackvec ={};
      trackvec.push_back(detvec[detct]);
