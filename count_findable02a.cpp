...
{
  cerr << "Usage: count_finddable02a -dets detfile -maxtime max inter-image time interval (hr)/ \n";
  cerr << "-maxvel maximum angular velocity (deg/day) -mintracktime min tracklet time span (sec)\n";
  cerr << "-outfile output file\n";
  cerr << "\nor, at minimum\n\n";
  cerr << "count_findable02a -dets detfile -outfile output file\n";
...
int main(int argc, char *argv[])
{
  ...
  while(detct<detvec.size()) {
	...
	if(currentstep >= INTRANIGHTSTEP) {
      ...
      for(i=1;i<long_trackletmjd.size(); i++) {
